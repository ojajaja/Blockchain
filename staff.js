...
var Contracts = { HouseRegistrationContract:  {
    ...
    address: "0xfe2c9e0d50024caab1f924b5eb80ab9551a9f205",
...
//                 console.log(error)
//             }
//             $("#message").text("Insurance Count: " + houseCount);
...
HouseRegistrationApp.prototype.registerNewHouse = function(){
    ...
    if (window.location.pathname === "/term.html") {
        var newAmount = 200;
      } else if (window.location.pathname ==="/whole.html"){
        var newAmount = 1000;
      }
        else if (window.location.pathname ==="/guaranteed.html"){
        var newAmount = 1500;
      }
        else if (window.location.pathname ==="/burial.html"){
        var newAmount = 300;
      }
        else {
    ...
    var that = this;
    this.getBalance(newAddress, function(error, balance){
        if(error){
            console.log(error);
        }
        else{
            if(balance.toNumber() > newAmount){
                that.instance.registerNewHouse(newOwner, newAddress, newAmount,
...
HouseRegistrationApp.prototype.transferHouse = function(){
    ...
    $("#message").text("Transferring Insurance No. " + txfHouseNo + " to " + txfOwner);

    var that = this;
    this.getBalance(txfAddress, function(error, balance){
        if(error){
            console.log(error);
        }
        else{
            if(balance.toNumber() > txfAmount){
                that.instance.transferHouse(txfHouseNo, txfOwner, txfAddress, txfAmount,
                ...
                function(){
                    ...
                    $("#message").text("Insufficient balance. Transfer Failed");
                }
            }
        }
        )
